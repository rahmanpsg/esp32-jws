...
boolean DMD::stepSplitMarquee(int topRow, int bottomRow, int StartColumn)
{
    ...
    if (marqueeOffsetX - StartColumn < -marqueeWidth)
    {                                                                                                                           // if text has scrolled off the screen
    ...
    { // loop for each row in the scroll area
        for (int byteInRow = (StartColumn / 8); byteInRow < DisplaysWide * 4; byteInRow++)
        {                                                                                                   // loop for each byte within a row (4 per DMD across)
            int thisIndex = (DisplaysTotal * 4) * (row % 16) + (4 * DisplaysWide * (row / 16)) + byteInRow; // calculate index into screen buffer
            if ((byteInRow % (DisplaysWide * 4)) == (DisplaysWide * 4) - 1)
            { // if it's the last byte in a row
                bDMDScreenRAM[thisIndex] = (bDMDScreenRAM[thisIndex] << 1) + 1;
                // shift bits left, and puts a '1' in last position (a '1' = LED OFF)
            }
            else
            { // if it's NOT the last byte in a row
